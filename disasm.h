...
#define NOSTR
#define BUILTIN_MEMSET

#if !defined(NOSTR)
#define MNEMONIC_STR(x) x,
#else
#define MNEMONIC_STR(x)
#endif

// Data types
#if _WIN32
#include <ntddk.h>
typedef unsigned __int64 QUSHORT;		// for MSVC
typedef signed   __int8  SUCHAR;
typedef signed   __int16 SUSHORT;
typedef signed   __int32 SULONG;
typedef signed   __int64 SQUSHORT;
#define snprintf _snprintf
#else
typedef u_int8_t  UCHAR;
typedef u_int16_t USHORT;
typedef u_int32_t ULONG;
typedef u_int64_t QUSHORT;
typedef int8_t    SUCHAR;
typedef int16_t   SUSHORT;
typedef int32_t   SULONG;
typedef int64_t   SQUSHORT;
#endif
...
typedef struct _OPERAND {
	...
	USHORT section;		// Section prefix value
	ULONG displacement;	// Displacement value
	ULONG immediate;	// Immediate value
...
typedef struct _INSTRUCTION {
	...
	UCHAR opcode;		// Actual opcode
	UCHAR modrm;		// MODRM byte
	UCHAR sib;		// SIB byte
...
int Disassemble(IN PUCHAR Address,
                OUT PINSTRUCTION pInstruction);

int get_instruction(
	INSTRUCTION *inst,	// pointer to INSTRUCTION structure
	UCHAR *addr,		// code buffer
...
int get_instruction_string(
	...
	ULONG offset,		// instruction absolute address
...
int get_operand_immediate(
	POPERAND op,
	ULONG *imm		// returned immediate value
);
int get_operand_displacement(
	POPERAND op,
	ULONG *disp		// returned displacement value
);
